...
public class JwtAccessTokenRepository extends AbstractJwtTokenRepository {
  ...
  /**
   * Constructs new instance of <code>JwtAccessTokenRepository</code> class.
   *
  ...
  public JwtAccessToken generateToken(HttpServletRequest request) {
    ...
    return Objects.nonNull(signedJwt)
        ? new JwtAccessToken(HEADER_NAME, CONTENT_KEY, TYPE, signedJwt.serialize())
        : null;
  }
  ...
  public long getExpirationTime() {
    return TimeUnit.MINUTES.toSeconds(
        this.applicationConfiguration.jwt().accessTokenExpirationTime());
  }
}
