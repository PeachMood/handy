...
public class JwtRefreshTokenRepository extends AbstractJwtTokenRepository {
  ...
  /**
   * <p>Constructs new instance of <code>JwtRefreshTokenRepository</code> class.</p>
   *
  ...
  public JwtRefreshToken generateToken(HttpServletRequest request) {
    ...
    return Objects.nonNull(signedJwt) ? new JwtRefreshToken(COOKIE_NAME, CONTENT_KEY, TYPE, signedJwt.serialize()) : null;
  }
  ...
  public void saveToken(AbstractJwtToken token, HttpServletRequest request, HttpServletResponse response) {
    Cookie cookie = Objects.nonNull(token) ? createJwtRefreshTokenCookie(token) : deleteJwtRefreshTokenCookie();
  ...
  public long getExpirationTime() {
    return TimeUnit.DAYS.toSeconds(this.applicationConfiguration.jwt().refreshTokenExpirationTime());
  }

  /* Creates JWT refresh token cookie */
  private Cookie createJwtRefreshTokenCookie(AbstractJwtToken abstractToken) {
    JwtRefreshToken refreshToken = abstractToken instanceof JwtRefreshToken ? (JwtRefreshToken) abstractToken : null;
