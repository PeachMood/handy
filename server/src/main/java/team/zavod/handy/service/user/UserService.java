...
public class UserService implements UserDetailsService {
  ...
  /**
   * <p>Constructs new instance of <code>UserService</code> class.</p>
   *
   ...
   * @return <code>true</code> if UserRepository was changed as a result of this call,
  ...
  public boolean createUser(UserEntity user) {
    ...
    user.setRoles(Stream.ofNullable(this.roleService.findRole(RoleEntity.DEFAULT_ROLE, RoleEntity.class)).collect(Collectors.toCollection(HashSet::new)));
   ...
   * @return User with the specified id if such user exists,
   * or <code>null</code> otherwise.
   */
  public <T> T findUser(Long id, Class<T> type) {
    return this.userRepository.findById(id, type)
        .orElse(null);
  }
   ...
   * @param type Class to be returned.
   * @param <T> Type parameter for returning class.
   * @return User with the specified login if such user exists,
   * or <code>null</code> otherwise.
   */
  public <T> T findUser(String login, Class<T> type) {
    return this.userRepository.findByUsername(login, type)
  ...
  /**
   * <p>Updates user with the specified data.</p>
   *
  ...
  public boolean updateUser(UserEntity with) {
    ...
    if((!with.getUsername().equals(current.getUsername()) && isUsernameExists(with.getUsername())) || (!with.getEmail().equals(current.getEmail()) && isEmailExists(with.getEmail()))) {
      return false;
    }
  ...
  /**
   * <p>Locates the user based on the username.</p>
   *
